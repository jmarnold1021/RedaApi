 ...
 *  1. TestCase and Project required in the body. <br>
 *  2. RunID is not allowed in the body.
 * @apiExample {curl} Example usage:
 *    #Optional: -H "Authorization: Basic ***"
 *    curl -H "Content-Type: application/json" \
 *            "http://<%HOSTNAME%>/api/v0/run" -d "@body.json"
 * @apiParam (JSON Body) {String} Project Project or Team who Performed the Run 
 ...
 * @apiParam (Query) {String} [Project] Project or Team who Performed the Run 
...
/**
 * @api {post} /run/:RunID/jamaReqs Insert Jama Run Requirments 
 * @apiVersion 0.0.1
 * @apiGroup Jama Requirements
 * @apiPermission <%USER_ROLE%>
 * @apiDescription Link a Jama Requirements to the specificed Run.
 * Accepts an array of ReqID Objects<br>
 * Jama data is synced to ReDa nightly. Any updates to<br>
 * Requirements through Jama may not be available until the next day
 * @apiExample {curl} Example usage:
 *    #Optional: -H "Authorization: Basic ***"
 *    curl -X POST "http://<%HOSTNAME%>/api/v0/run/567/jamaReqs" -d "@body.json"
 * @apiParamExample {json} Jama Reqs Bulk:
 *   [
 *     {
 *       "ReqID": "req1"
 *     },
 ...
 *   ]
 * @apiParam (Path) {Number} RunID Unique Run ID
 * @apiParam (JSON Body) {String} ReqID Requirment ID determined in Jama
 */


/**
 * @api {post} /run/:RunID/jamaReq/:ReqID Insert Jama Run Requirment 
 * @apiVersion 0.0.1
 * @apiGroup Jama Requirements
 * @apiPermission <%USER_ROLE%>
 * @apiDescription Link a Jama Requirement to the specificed Run.
 * <b>Jama data is synced to ReDa nightly. Any updates to<br>
 * Requirements through Jama may not be available until the next day<b>
 * @apiExample {curl} Example usage:
 *    #Optional: -H "Authorization: Basic ***"
 *    curl -X POST "http://<%HOSTNAME%>/api/v0/run/567/jamaReq/REQ1"
 * @apiParam (Path) {Number} RunID Unique Run ID
 * @apiParam (Path) {String} ReqID Requirment ID determined in Jama
 */


/**
 * @api {post} /run/:RunID/step/:StepID/jamaReqs Insert Jama Step Requirments
 * @apiVersion 0.0.1
 * @apiGroup Jama Requirements
 * @apiPermission <%USER_ROLE%>
 * @apiDescription Link a Jama Requirment to the specificed Step.
 * Accepts an array of ReqID Objects<br>
 * Jama data is synced to ReDa nightly. Any updates to<br>
 * Requirements through Jama may not be available until the next day
 * @apiExample {curl} Example usage:
 *    #Optional: -H "Authorization: Basic ***"
 *    curl -X POST "http://<%HOSTNAME%>/api/v0/run/567/step/1/jamaReqs" -d "@body.json"
 * @apiParamExample {json} Jama Reqs Bulk:
 *   [
 *     {
 *       "ReqID": "req1"
...
/**
 * @api {post} /run/:RunID/step/:StepID/jamaReq/:ReqID Insert Jama Step Requirment 
 * @apiVersion 0.0.1
 * @apiGroup Jama Requirements
 * @apiPermission <%USER_ROLE%>
 * @apiDescription Link a Jama Requirment to the specificed Step.
 ...
 * @apiExample {curl} Example usage:
 *    #Optional: -H "Authorization: Basic ***"
 *    curl -X POST "http://<%HOSTNAME%>/api/v0/run/567/step/1/jamaReq/REQ1"
 * @apiParam (Path) {Number} RunID Unique Run ID
 * @apiParam (Path) {Number} StepID Ordered(1,2,...) Step ID
 * @apiParam (Path) {String} ReqID Requirment ID determined in Jama
 */
 ...
 * @api {get} /jamaReqs Get Jama Requirements
 * @apiVersion 0.0.1
 * @apiGroup Jama Requirements
 * @apiPermission <%USER_ROLE%>
 * @apiDescription Retrieve Jama Requirements matching the specified query parameters(In Dev).
 * @apiExample {curl} Example usage:
 *    #Optional: -H "Authorization: Basic ***"
 *    curl "http://<%HOSTNAME%>/api/v0/jamaReqs?ReqID=jamaReqId"
 * @apiParam (Query) {Number} [RunID] Unique Run ID
 * @apiParam (Query) {Number} [StepID] Ordered(0,1,2,...) Step ID
 * @apiParam (Query) {String} [ReqID] Requirment ID determined in Jama
 */
...
/**
 * @api {post} /run/:RunID/metric Insert Run Metric
 * @apiVersion 0.0.1
 * @apiGroup Metrics
 * @apiPermission <%USER_ROLE%>
 * @apiDescription Create a new Metric.
 * @apiExample {curl} Example usage:
 *    #Optional: -H "Authorization: Basic ***"
 *    curl -H "Content-Type: application/json" \
 *            "http://<%HOSTNAME%>/api/v0/run/7/metric" -d "@body.json"
 * @apiParamExample {json} Metric with Params:
 *   {
 *      "Type": "float",
 *      "Metric": "cup_time",
 *      "Value": 8899339,
 ...
 *   }
 * @apiParam (Path) {Number} RunID Unique Run Id.
 * @apiParam (JSON Body) {String} [Datetime='UTC NOW'] Time the metrics was measured 
 * @apiParam (JSON Body) {String} Type the type of metric
 * @apiParam (JSON Body) {String} Metric The actual measurment ('elapsed_time')
 * @apiParam (JSON Body) {String} Value The value of the metric. 
 * @apiParam (JSON Body) {Object} [Params] An array or a single Parameter
 */

/**
 * @api {post} /run/:RunID/step/:StepID/metric Insert Step Metric
 * @apiVersion 0.0.1
 * @apiGroup Metrics
 * @apiPermission <%USER_ROLE%>
 ...
 *    #Optional: -H "Authorization: Basic ***"
 *    curl -H "Content-Type: application/json" \
 *            "http://<%HOSTNAME%>/api/v0/run/7/step/1/metric" -d "@body.json"
 * @apiParamExample {json} Metric with Params:
 *   {
 *      "Type": "float",
 *      "Metric": "cup_time",
 ...
 * @apiParam (Query) {Number} [RunID] Unique Run Id
 * @apiParam (Query) {Number} [StepID] Ordered(1,2,...) Step Id
 * @apiParam (Query) {Number} [MetricID] Unique MetricID.
 * @apiParam (Query) {String} Type the type of metric
 * @apiParam (Query) {String} Metric The actual measurment ('elapsed_time')
 * @apiParam (Query) {Number} Value The value of the metric.
 */

/**
 * @api {post} /metric/:MetricID/param Insert Metric Parameters 
 * @apiVersion 0.0.1
 * @apiGroup Parameters
 * @apiPermission <%USER_ROLE%>
 * @apiDescription Create metric parameters.<br>
 * Accepts and Array of Param objects
 * @apiExample {curl} Example usage:
 *    #Optional: -H "Authorization: Basic ***"
 *    curl -H "Content-Type: application/json" \
 *            "http://<%HOSTNAME%>/api/v0/metric/7/param" -d "@body.json"
 * @apiParamExample {json} Metric with Params:
 *   [
 *     {
 *       "Name": "Param1",
 ...
 *     }
 *   ]
 * @apiParam (Path) {Number} MetricID Unique Metric Id.
 * @apiParam (JSON Body) {String} Name The Parameter Name.
 * @apiParam (JSON Body) {String} Value The Value of the Parameter.
 */
